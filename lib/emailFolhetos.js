...
function semanaAtual() {
  return semanaAtualCompartilhada().semana;
}
...
export function construirEmailFolhetos({ nome, folhetos, base, unsubscribeUrl, resumo = null }) {
  ...
  const linhasHtml = folhetos.map(f => {
    ...
    // Sem "válido de X a Y" por loja: cada cadeia tem o seu período e nós
    // não o sabemos. A semana já está no cabeçalho do email, como contexto.
    const validade = "Datas no folheto oficial";
    return `
      <tr>
  ...
  if (resumo && (resumo.totalMes > 0 || resumo.streak >= 2 || (resumo.garantias || []).length)) {
    ...
    if (resumo.totalMes > 0) {
      partes.push(`<td align="center" style="padding:10px 6px;">
        <div style="font-size:20px;font-weight:900;color:#0b6b4f;">€${eur(resumo.totalMes, 2)}</div>
