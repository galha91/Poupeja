 ...
 */
import { todosOsPostos, normalizarPosto, postoUtilizavel, haversine, minimoPorMarca, semPrecosAbsurdos } from "./dgeg";
 ...
 *   2. snapshot no Supabase     — o caminho normal; mantido uma vez por dia
...
async function obterPostos() {
  const lote = await obterLote();
  /*
   * O filtro dos preços impossíveis fica AQUI, depois de escolhida a
   * fonte, e não dentro de cada ramo — senão o caminho do snapshot (que
   * é o normal em produção) passava ao lado dele, que foi precisamente
   * o erro que estive quase a cometer. Filtrar na leitura em vez de na
   * escrita também quer dizer que um snapshot já gravado com lixo fica
   * limpo à mesma, sem esperar pelo cron seguinte.
   */
  const { postos, removidos } = semPrecosAbsurdos(lote.postos);
  if (removidos.length) {
    console.warn(`municipios: ${removidos.length} preço(s) fora do plausível ignorados (origem: ${lote.origem}) —`,
...
/*
 * Preços mínimos por marca, para a página nacional.
 *
 * Usa a mesma cadeia de origem que a lista de concelhos (snapshot →
 * DGEG), por isso as duas metades da página passam a vir do mesmo sítio.
 */
export async function precosPorMarca() {
  const { postos, obtidoEm, dataPreco, stale } = await obterPostos();
  return { dados: minimoPorMarca(postos), frescura: { obtidoEm, dataPreco, stale } };
}
