...
let cache = null; // { lista, medias, ts }
...
async function carregar() {
  ...
  const postos = (await todosOsPostos()).map(normalizarPosto).filter(postoUtilizavel);
...
export async function dadosMunicipio(slug, { raioKm = 20 } = {}) {
  const { lista, medias } = await carregar();
