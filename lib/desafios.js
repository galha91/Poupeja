...
export /*
 * Cada mês tinha uma `cor` própria — roxo, rosa, cyan, indigo... doze
 * tons do Tailwind, nenhum da paleta do PoupeJá, a pintar a barra de
 * progresso e os botões. Nada em setembro quer dizer indigo: a cor
 * mudava de identidade todos os meses sem dizer nada a ninguém. O que
 * distingue o desafio é o nome e o símbolo; a cor de poupar aqui é
 * uma só, a da marca.
 */
const DESAFIOS_MENSAIS = [
  { mes: 0,  nome: "Ano Novo, Carteira Cheia", emoji: "🎯", meta: 30, dica: "Aproveita os saldos de janeiro só para o que precisas mesmo." },
  { mes: 1,  nome: "Fevereiro Poupado",        emoji: "💘", meta: 25, dica: "Mês curto, poupança rápida — compara folhetos antes de cada compra." },
  { mes: 2,  nome: "Operação Primavera",       emoji: "🌸", meta: 25, dica: "Os legumes da época custam menos e rendem mais." },
  { mes: 3,  nome: "Páscoa sem Estragos",      emoji: "🐣", meta: 30, dica: "Compra o chocolate na semana a seguir à Páscoa — desce até 50%." },
  { mes: 4,  nome: "Maio a Render",            emoji: "🌞", meta: 25, dica: "Faz a lista antes de sair de casa. Quem leva lista gasta menos." },
  { mes: 5,  nome: "Junho dos Campeões",       emoji: "🏖️", meta: 25, dica: "As marcas brancas têm a mesma fábrica e metade do preço." },
  { mes: 6,  nome: "Férias Inteligentes",      emoji: "✈️", meta: 30, dica: "Antes das férias, gasta primeiro o que já tens na despensa." },
  { mes: 7,  nome: "Agosto sem Aperto",        emoji: "⛱️", meta: 30, dica: "Compra fruta da época — agosto é o mês mais barato do ano." },
  { mes: 8,  nome: "Regresso em Grande",       emoji: "🎒", meta: 35, dica: "Material escolar: compara os folhetos, as diferenças chegam a 40%." },
  { mes: 9,  nome: "Outubro dos Descontos",    emoji: "🍂", meta: 25, dica: "Guarda o talão de TODAS as compras — o que não se mede, não se poupa." },
  { mes: 10, nome: "Black November",           emoji: "🛍️", meta: 40, dica: "Na Black Friday, só é desconto se já querias comprar antes." },
  { mes: 11, nome: "Natal sem Dívidas",        emoji: "🎄", meta: 40, dica: "Define o orçamento de presentes ANTES de entrar nas lojas." },
];
...
export async function partilharDesafio(estado) {
  ...
  const texto = estado.completo
    ? `${desafio.emoji} Completei o desafio "${desafio.nome}" do PoupeJá — poupei €${eur(totalMes, 2)} nas compras este mês! 💪 Achas que consegues? Experimenta grátis:`
...
async function partilharTexto(texto) {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: "PoupeJá", text: texto, url: urlApp() });
      return "partilhado";
    } catch (e) {
  ...
  try {
    await navigator.clipboard.writeText(`${texto} ${urlApp()}`);
    return "copiado";
  } catch {}
  return false;
}
