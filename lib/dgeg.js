...
/*
 * Idade máxima que aceitamos servir quando a DGEG falha. Passado isto,
 * preferimos não ter preços a ter preços velhos: numa página que diz o
 * que custa o gasóleo hoje, um número errado é pior do que número nenhum.
 */
export const IDADE_MAXIMA = 6 * 60 * 60 * 1000;
...
export async function postosDoDistrito(id, { timeoutMs = 7000 } = {}) {
  ...
  if (guardado && agora - guardado.ts < CACHE_TTL) {
 ...
 */
export async function juntarDistritos(ids) {
...
export async function todosOsPostos() {
  return juntarDistritos(DISTRITOS.map(d => d.id));
}
