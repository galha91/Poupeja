...
/*
 * Até quando é que um preço ainda vale a pena ser mostrado.
 *
 * Estive nas 6 horas e era exagero: ao fim disso a página dava 404 e o
 * utilizador não via NADA, quando o que tínhamos eram preços reais da
 * DGEG, apenas com umas horas. Um preço de ontem, com a data à vista,
 * ainda diz qual é o posto barato do concelho — uma página em branco não
 * diz nada. Esconder só se justifica quando o número já não descreve o
 * mercado, e aí uma semana é a fronteira honesta.
 *
 * Isto NÃO é licença para mentir: seja qual for a idade, a data aparece
 * no cabeçalho, o título deixa de dizer "hoje" e há um aviso a dizer de
 * quando são os preços. O que muda é mostrar-se em vez de esconder.
 *
 * Uma constante só, partilhada com o snapshot, para não haver dois
 * prazos a discordar um do outro.
 */
export const VALIDADE_PRECOS = 7 * 24 * 60 * 60 * 1000;
export const IDADE_MAXIMA = VALIDADE_PRECOS;
...
export async function postosDoDistrito(id, { timeoutMs = 7000, forcar = false } = {}) {
  ...
  function recorrerACache() {
    ...
    if (guardado && agora - guardado.ts < IDADE_MAXIMA) {
