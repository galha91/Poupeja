...
import { Search, X, ChevronLeft, Trophy, Flame, TrendingDown } from "lucide-react";
...
/* ═══ LOGO DA LOJA (logótipo real Clearbit + fallback) ════ */
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col overflow-y-auto">
      <div className="bg-white border-b border-slate-100 px-4 pt-12 pb-4 sticky top-0 z-10 shadow-sm">
        <button onClick={onVoltar} className="flex items-center gap-1 text-blue-600 font-bold text-sm mb-3">
      ...
      <div className="px-4 py-4 flex flex-col gap-4 pb-10">
        <div className="rounded-2xl p-4 text-white shadow-lg" style={{ background: "linear-gradient(135deg,#059669,#10b981)" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              {melhor.emPromo && <span className="inline-block mt-0.5 text-[9px] font-black bg-white/20 px-2 py-0.5 rounded-full flex items-center gap-1 w-fit"><Flame size={9}/> Promoção</span>}
            </div>
            <div className="text-right">
              <p className="text-2xl font-black">{min.toFixed(2)}€</p>
              <p className="text-xs opacity-75">Poupa {(max - min).toFixed(2)}€</p>
            </div>
          </div>
        </div>

        <div>
          <p className="text-xs font-black text-slate-500 uppercase tracking-wider mb-3">Comparação por supermercado</p>
            ...
            {precos.map((p, i) => {
              ...
              return (
                <div key={p.loja} className="bg-white rounded-2xl p-4 border shadow-sm"
                  style={{ borderColor: isBest ? "#10b981" : "#f1f5f9", boxShadow: isBest ? "0 0 0 1px #10b98144" : undefined }}>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0"
                      style={{ backgroundColor: isBest ? "#10b981" : "#f1f5f9", color: isBest ? "white" : "#94a3b8" }}>{i + 1}</div>
                    <LogoLoja loja={p.loja} size={36} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="font-bold text-sm text-slate-800">{p.loja}</p>
                        {isBest && <span className="text-[9px] font-black bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full flex items-center gap-0.5"><Trophy size={8}/> Mais barato</span>}
                    ...
                    <p className="text-xl font-black flex-shrink-0" style={{ color: isBest ? "#059669" : "#1e293b" }}>{p.valor.toFixed(2)}€</p>
                  </div>
                  ...
                  {!isBest && <p className="text-[10px] text-slate-400 text-right">+{(p.valor - min).toFixed(2)}€ vs. mais barato</p>}
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
          <p className="text-xs font-black text-slate-500 uppercase tracking-wider mb-3">Comparação visual</p>
...
function CardProduto({ produto, onClick }) {
  ...
  return (
    <button onClick={onClick} className="w-full bg-white rounded-2xl p-4 border border-slate-100 shadow-sm text-left active:scale-95 transition-all">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            ...
            <p className="text-[11px] text-slate-500"><span className="font-black" style={{ color: SUPER_CORES[melhor.loja] }}>{melhor.loja}</span> — mais barato</p>
          </div>
        ...
        <div className="text-right flex-shrink-0 ml-2">
          <p className="text-lg font-black text-emerald-600">{min.toFixed(2)}€</p>
    ...
    <div className="pb-6">
      <div className="mx-4 mb-4 rounded-2xl overflow-hidden shadow-lg" style={{ background: "linear-gradient(135deg,#1d4ed8,#2563eb)" }}>
        <div className="px-4 pt-4 pb-3 text-white">
          <div className="flex items-center gap-2 mb-1"><TrendingDown size={18}/><span className="text-[10px] font-black uppercase tracking-widest opacity-70">Comparador de Preços</span></div>
      ...
      <div className="px-4 mb-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Pesquisar produto... ex: leite, frango" value={busca} onChange={e => setBusca(e.target.value)}
            className="w-full pl-9 pr-9 py-3 rounded-xl bg-white border border-slate-200 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" />
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {CATEGORIAS.map(cat => (
            <button key={cat} onClick={() => setCategoria(cat)} className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold transition-all"
              style={{ backgroundColor: categoria === cat ? "#2563eb" : "white", color: categoria === cat ? "white" : "#64748b", border: categoria === cat ? "none" : "1px solid #e2e8f0" }}>{cat}</button>
          ))}
        </div>
      </div>
      ...
      {filtrados.length === 0 && (
        <div className="mx-4 bg-amber-50 border border-amber-100 rounded-2xl p-6 text-center">
          <Search size={32} className="text-amber-500 mx-auto mb-2"/>
      ...
      {filtrados.length > 0 && <div className="px-4 mb-2"><p className="text-xs text-slate-400 font-semibold">{filtrados.length} produto{filtrados.length !== 1 ? "s" : ""}{categoria !== "Todas" ? ` em ${categoria}` : ""}</p></div>}
