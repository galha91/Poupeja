...
function BadgeCusto({ custo }) {
  return (
    <span className="font-semibold" style={{ fontSize: 12, color: "var(--pj-brand-ink)" }}>
  ...
  return (
    <button
      ...
      style={destaque
        ...
        : { background: "var(--pj-card)", border: "1px solid var(--pj-border)" }}
    >
      ...
      <span style={{ flex: 1, minWidth: 0 }}>
        ...
        <span className="block truncate font-semibold" style={{ fontSize: 14.5, color: destaque ? "#fff" : "var(--pj-text)" }}>
          {ementa.nome}
        </span>
        <span className="flex items-center gap-2.5 mt-0.5" style={{ fontSize: 12, color: destaque ? "#a7cbbb" : "var(--pj-text-muted)" }}>
  ...
  return (
    <div>
      ...
      <div className="mx-4 mb-4">
        <p className="text-[11px] font-semibold uppercase tracking-[0.09em]" style={{ color: "var(--pj-text-faint)" }}>Cozinha económica</p>
        <p className="font-display" style={{ fontSize: 22, fontWeight: 600, color: "var(--pj-text)", marginTop: 4 }}>Ementas da semana</p>
        <p className="text-xs mt-1" style={{ color: "var(--pj-text-muted)" }}>
          Receitas baratas para 4 pessoas — os ingredientes entram na tua lista num toque.
        </p>
        <div className="mt-4" style={{ height: 1, background: "var(--pj-border)" }} />
      </div>
      ...
      <div className="px-4 mb-4 flex gap-2 overflow-x-auto no-scrollbar">
        {FILTROS.map(f => (
          <button key={f.id} onClick={() => setFiltro(f.id)}
            ...
            style={filtro === f.id ? { background: "var(--pj-brand)", color: "#fff" } : { background: "var(--pj-subtle)", color: "var(--pj-text-muted)" }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <p className="text-[11px] text-center px-6 pb-4 leading-relaxed" style={{ color: "var(--pj-text-faint)" }}>
      ...
      {aberta && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(20,35,28,0.55)", backdropFilter: "blur(3px)" }} onClick={() => setAberta(null)}>
          <div className="w-full max-w-md rounded-t-3xl px-5 pt-5 pb-9" style={{ maxHeight: "90vh", overflowY: "auto", background: "var(--pj-surface)" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3 mb-1">
              <div className="flex items-center gap-3">
                <span style={{ fontSize: 34 }}>{aberta.emoji}</span>
                <div>
                  <p className="font-display" style={{ fontSize: 19, fontWeight: 600, color: "var(--pj-text)", lineHeight: 1.2 }}>{aberta.nome}</p>
                  <p className="flex items-center gap-2.5 mt-1" style={{ fontSize: 12, color: "var(--pj-text-muted)" }}>
              ...
              <button onClick={() => setAberta(null)} aria-label="Fechar" className="pj-tap w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "var(--pj-subtle)" }}>
            ...
            {adicionado !== 0 ? (
              <div className="mt-4">
                <div className="flex items-center justify-center gap-2 py-3 rounded-2xl" style={{ background: "var(--pj-brand-wash)" }}>
                  <Check size={15} style={{ color: "var(--pj-brand-ink)" }} />
                  <span className="text-[13px] font-semibold" style={{ color: "var(--pj-brand-ink)" }}>
                    {adicionado === -1 ? "Já estavam todos na lista" : `${adicionado} ingrediente${adicionado !== 1 ? "s" : ""} na lista`}
                  </span>
                </div>
                <button onClick={() => setTab?.("lista")} className="pj-tap w-full py-3 mt-2 rounded-2xl font-semibold text-[13.5px]" style={{ background: "var(--pj-brand)", color: "#fff" }}>
                  Ver a lista de compras →
                </button>
              </div>
            ) : (
              <button onClick={adicionar} className="pj-tap w-full py-3.5 mt-4 rounded-2xl text-white font-semibold flex items-center justify-center gap-2" style={{ background: "var(--pj-brand)" }}>
                <ShoppingCart size={16} /> Adicionar {naoDespensa} ingredientes à lista
              </button>
            )}

            <p className="text-[11px] font-semibold uppercase tracking-[0.09em] mt-5 mb-2" style={{ color: "var(--pj-text-faint)" }}>Ingredientes</p>
            <div className="rounded-2xl overflow-hidden" style={{ background: "var(--pj-card)", border: "1px solid var(--pj-border)" }}>
              {aberta.ingredientes.map((ing, i) => (
                <div key={i} className="flex items-center justify-between px-4 py-2.5" style={i > 0 ? { borderTop: "1px solid var(--pj-subtle)" } : {}}>
                  <span style={{ fontSize: 13.5, fontWeight: ing.despensa ? 500 : 600, color: ing.despensa ? "var(--pj-text-faint)" : "var(--pj-text)" }}>
                    {emojiIngrediente(ing.nome)} {ing.nome}{ing.despensa ? " · da despensa" : ""}
                  </span>
                  <span style={{ fontSize: 12.5, color: "var(--pj-text-muted)" }}>{ing.qtd}</span>
                </div>
              ))}
            </div>

            <p className="text-[11px] font-semibold uppercase tracking-[0.09em] mt-5 mb-2" style={{ color: "var(--pj-text-faint)" }}>Preparação</p>
            <ol className="flex flex-col gap-2.5" style={{ paddingLeft: 0, listStyle: "none" }}>
              {aberta.passos.map((p, i) => (
                <li key={i} className="flex gap-3">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full text-white text-[11px] font-semibold flex items-center justify-center mt-0.5" style={{ background: "var(--pj-brand)" }}>{i + 1}</span>
                  <span style={{ fontSize: 13.5, color: "var(--pj-text-strong)", lineHeight: 1.55 }}>{p}</span>
                </li>
              ))}
            </ol>
