    ...
    <div className="px-4 mb-5 anim-up anim-up-1">
      {!meta ? (
        <button onClick={abrirForm} className="pj-tap press w-full text-left flex items-center rounded-2xl px-4 py-3.5"
          style={{ gap: 12, background: "#fbfaf6", border: "1.5px dashed #0b6b4f55" }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "#eeece4" }}>
            <Target size={18} style={{ color: "#0b6b4f" }} />
          </div>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 13.5, fontWeight: 600, color: "#14231c" }}>Estou a poupar para…</p>
            <p style={{ fontSize: 12, color: "#5c6b62", marginTop: 1 }}>Define uma meta e vê a poupança real a enchê-la</p>
          </div>
        </button>
      ) : (
        <div className="rounded-2xl p-4" style={{ background: completa ? "#eef3ef" : "#fbfaf6", border: `1px solid ${completa ? "#0b6b4f44" : "#e4e2d8"}` }}>
          <div className="flex items-center" style={{ gap: 12 }}>
            <span style={{ fontSize: 26, flexShrink: 0 }}>{meta.emoji}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p className="truncate" style={{ fontSize: 14, fontWeight: 600, color: "#14231c" }}>{meta.nome}</p>
              <p style={{ fontSize: 12, color: "#5c6b62", marginTop: 1 }}>
                {completa
            ...
            <span className="font-display flex-shrink-0" style={{ fontSize: 19, fontWeight: 600, color: "#0b6b4f" }}>{pct}%</span>
            <button onClick={abrirForm} aria-label="Editar meta" className="pj-tap w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#eeece4" }}>
              <Pencil size={13} style={{ color: "#5c6b62" }} />
            </button>
          </div>
          <div className="mt-3" style={{ height: 6, borderRadius: 4, background: "#e4e2d8", overflow: "hidden" }}>
            <div className="pj-bar" style={{ height: "100%", width: `${pct}%`, "--pj-final-width": `${pct}%`, background: "#0b6b4f", borderRadius: 4 }} />
          </div>
          {completa && (
            <button onClick={remover} className="pj-tap mt-3 inline-flex items-center gap-1.5 text-[12px] font-semibold" style={{ color: "#0b6b4f" }}>
      ...
      {editar && (
        <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(20,35,28,0.55)" }} onClick={() => setEditar(false)}>
          <form onSubmit={submeter} className="w-full rounded-t-3xl px-5 pt-5 pb-9" style={{ background: "#fbfaf6" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <p className="font-display" style={{ fontSize: 18, fontWeight: 600, color: "#14231c" }}>{meta ? "Editar meta" : "Nova meta de poupança"}</p>
              <button type="button" onClick={() => setEditar(false)} aria-label="Fechar" className="pj-tap w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "#eeece4" }}>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {EMOJIS.map(e => (
                <button key={e} type="button" onClick={() => setFEmoji(e)}
                  ...
                  style={fEmoji === e ? { border: "1.5px solid #0b6b4f", background: "#eef3ef" } : { border: "1px solid #e4e2d8", background: "#f6f5f0" }}>
                  {e}
                </button>
              ))}
            </div>
            ...
            <input type="number" inputMode="decimal" step="0.01" min="1" placeholder="Quanto? (ex: 300)" value={fValor} onChange={e => setFValor(e.target.value)} required
              ...
              style={{ background: "#f6f5f0", border: "1px solid #e4e2d8", color: "#14231c" }} />
            <button type="submit" className="pj-tap w-full py-4 rounded-2xl text-white font-semibold" style={{ background: "#0b6b4f" }}>
